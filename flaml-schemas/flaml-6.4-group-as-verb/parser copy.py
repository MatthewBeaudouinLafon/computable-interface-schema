...
def spec_from_file(file_path):
    real_path = Path(__file__).with_name(
        file_path
...
class rel(Enum):
    ...
    CREATE = "CREATE"
    DELETE = "DELETE"
    UPDATE = "UPDATE"  # if the update applies to a set
    UPDATE_SRC = "UPDATE_SRC"  # if the update applies to a relation
    UPDATE_TRG = "UPDATE_TRG"  # if the update applies to a relation
    DIRECTION = "DIRECTION"
...
def declare(
    ...
    assert (
        source is not None
    ), f"`source` cannot be None in: {source}  -{pretty_relation}->  {target}"
    assert (
        relation is not None
    ), f"`relation` cannot be None in: {source}  -{pretty_relation}->  {target}"
    assert (
        target is not None
    ), f"`target` cannot be None in: {source}  -{pretty_relation}->  {target}"
    if verbose:
...
def update_declaration_power(edge: tuple, power: dpower):
    assert (
        type(power) is dpower
    ), f"Type Error: expected `dpower`, got `{type(power)}` instead."
...
def substitute_instance_name_in_decl(edge: tuple, instance_name: str):
    assert (
        type(instance_name) is str
    ), f"Type Error: expected `str`, got `{type(instance_name)}` instead."
    return (
        sub_inst_name_in_node(edge[0], instance_name),
        edge[1],
        sub_inst_name_in_node(edge[2], instance_name),
        edge[3],
    )
...
def validate_instance_dict(statement: str):
    ...
    ), f"Type Error: `{statement}` should be a string"
    is_relation = parse_relation(statement) is not None
    is_attribute = statement[0] == "/"
    is_valid = is_relation or is_attribute
    assert (
        is_valid
    ), f"Key `{statement}` is an invalid key for an instance dict. \
  {is_relation=} or {is_attribute=}"
    return is_valid


def parse_compound_object(
    statement: str, parent: str, interp: tuple[list, list], path: SpecPath = []
):
    """
    ...
    for phrase in statement.split("->"):
        if phrase[0] == "/":
            assert (
                parent is not None
            ), f"`{phrase}` starts with `/`, but its parent is None."
...
def parse_str(
    ...
    if "." in statement or "->" in statement or "/" in statement:
        statement = parse_compound_object(
            statement=statement, parent=parent, interp=interp
        )
...
def parse_dict(
    ...
    for key, value in statement.items():
        ...
        # 2. If the key starts with /, then it declares: parent/key -mapto-> value.
            ...
            if type(value) is str:
                ...
                parsed_value = parse_dict(
                    value,
                    key_parent=next_key_parent,
                    val_parent=next_val_parent,
                    interp=interp,
                    depth=depth + 1,
                    path=[*path, key],
                )
        ...
        #    then its the parent to declaration involving the items in its value.
            ...
            if rel.GROUP_FOREACH in map(
                lambda x: parse_relation(x), value.keys()
            ):
                ...
                parse_dict(
                    value,
                    key_parent=parsed_key,
                    val_parent=parsed_key,
                    interp=interp,
                    depth=depth + 1,
                    path=[*path, key],
                )
            ...
            # TODO: probably use proper regex...
                ...
                for value_key in value.keys():
                    ...
                    assert (
                        type(value_key) is str
                    ), f"Type Error. Expected `str` or relation, and got `{type(value_key)}"
                    ...
                    assert (
                        value_key[0] == "/"
                    ), f"Values for instantiation should be /statements or a relation, got `{value_key}` instead."
            else:
...
def parse_type_def_str(
    statement: str, verbose=False
) -> tuple[str | None, str | None]:
    # NOTE: This will intentionally not match `def (my-type) my-instance`
    assert isinstance(
        statement, str
    ), f"Type Error: expected str, got {type(statement)} instead from {statement}"
    if statement[:4] != "def ":
        return (None, None)

    re_res = re.match(
        r"^def \((?P<type>[\w\-]+)\) extends \((?P<parent>[\w\-]+)\)$",
        statement,
    )
...
def prepare_target_relation_statement(statement: str):
    ...
    assert (
        len(split) == 3
    ), f"Expected relation statement two split into 3 (on space): {statement}"

    assert (
        "and" not in split
    ), f"Expected `relation>` statement, got an `and` statement: {statement}"
    ...
    assert (
        relation[-1] == ">"
    ), f"Expected `relation>` in the statement: {statement}"

    relation = relation[:-1]
    assert (
        parse_relation(relation) is not None
    ), f"`relation>` is not a relation in: {statement}"
...
def parse_target_relation(
    statement: str,
    inflicting_action: str,
    parent: str,
    interp: tuple[list, list],
    depth: int,
    verbose=False,
    ...
    raw_source, raw_relation, raw_target = prepare_target_relation_statement(
        statement
    )
    source = parse_str(
        raw_source, parent=parent, interp=interp, depth=depth + 1, path=path
    )
    target = parse_str(
        raw_target, parent=parent, interp=interp, depth=depth + 1, path=path
    )

    declare(
        interp=interp,
        source=inflicting_action,
        relation=rel.UPDATE_SRC,
        target=source,
        power=dpower.STRONG,
        verbose=verbose,
    )
...
def parse_type_definitions(spec, verbose=False) -> tuple:
    ...
    for statement in spec:
        ...
        if type(statement) is str:
            ...
            if verbose:
                print_parent = (
                    ...
                    else f" with parent: ({parent_type_name})"
                )
        ...
        # statement is dict
        assert (
            len(statement.keys()) == 1
        ), f"Found multiple keys in a type definition dict: {list(statement.keys())}"
        assert (
            len(statement.values()) == 1
        ), f"Found multiple values in a type definition dict: {list(statement.values())}"
        type_def_key, type_def_content = (
            list(statement.keys())[0],
            list(statement.values())[0],
        )
        ...
        assert (
            type(type_def_content) is dict
        ), f"Contents of type definition should be a dict, got this instead: {type_def_content}"

        type_name, parent_type_name = parse_type_def_str(type_def_key, verbose)
        assert (
            type_name is not None
        ), f"Failed to find type name in supposed type definition: `{statement}`"
        ...
        for relation in type_relations:
            ...
            ), f"Type Error: Expected str, got `{type(relation)}` instead from `{relation}`."
            parsed_rel = parse_relation(relation)
            assert parsed_rel in (
                rel.GROUP_FOREACH,
                rel.TBD,
            ...
            if parsed_rel == rel.GROUP_FOREACH:
                ...
                if type(group_foreach_content) is str:
                    ...
                    parse_list(
                        group_foreach_content,
                        key_parent=instance_name,
                        val_parent=instance_name,
                        interp=(type_interp, []),
                        depth=2,
                    )
                else:
                    assert (
                        False
                    ), f"Type Error: group_foreach_content should be str or list, got `{type(group_foreach_content)}` instead."
        ...
        for declaration in type_interp:
            weak_declaration = update_declaration_power(
                declaration, dpower.WEAK
            )
...
def make_relations(spec, verbose=False, locations=False):
    assert type(spec) is list, "Top level of YAML specification must be a list."

    interp = ([], [])
    parse_list(
        spec, key_parent=None, val_parent=None, interp=interp, depth=0, path=[]
    )
